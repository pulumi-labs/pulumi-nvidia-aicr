...
With `validate: true` (`pulumi config set validate true`) a `ValidationRun`
snapshots the cluster and runs the recipe's deployment and conformance checks
in-cluster (~10 minutes, needs provider/SDK >= 0.3.0). On a GPU-less kind
cluster expect an honest result: readiness passes, GPU deployment checks fail
or skip, GPU conformance checks skip. Useful for exercising the validation
pipeline itself — a real verdict needs real hardware (see the EKS/GKE
examples).
